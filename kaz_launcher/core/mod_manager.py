...
USER_AGENT = 'KazLauncher/1.0'
def _get_headers() -> dict:
    return {'User-Agent': USER_AGENT}
_SESSION = None
def get_session() -> requests.Session:
    """Sesión compartida para reutilizar conexiones TCP en descargas concurrentes."""
    global _SESSION
    if _SESSION is None:
        _SESSION = requests.Session()
        _SESSION.headers.update(_get_headers())
        retry = Retry(total=3, connect=5, read=3, backoff_factor=0.6, status_forcelist=(408, 429, 500, 502, 503, 504), raise_on_status=False)
...
def search_mods(query: str, game_version: str, loader: str, lang_dict: dict, sort_option: str='relevance', offset: int=0):
    facets = [[f'versions:{game_version}'], ['project_type:mod'], [f'categories:{loader.lower()}']]
    params = {'query': query, 'facets': json.dumps(facets), 'limit': 20, 'index': sort_option, 'offset': offset}
    try:
        response = request_with_retries('GET', f'{MODRINTH_API_URL}/search', params=params, headers=_get_headers(), timeout=10)
...
def get_project_details(project_id: str, lang_dict: dict):
    if not project_id:
        ...
        try:
            response = request_with_retries('GET', f'{MODRINTH_API_URL}/project/{project_id}', headers=_get_headers(), timeout=10)
            response.raise_for_status()
...
def get_latest_mod_version(project_id: str, game_version: str, loader: str, lang_dict: dict):
    params = {'game_versions': json.dumps([game_version]), 'loaders': json.dumps([loader.lower()])}
    try:
        response = request_with_retries('GET', f'{MODRINTH_API_URL}/project/{project_id}/version', params=params, headers=_get_headers(), timeout=10)
        response.raise_for_status()
...
def download_file(url: str, destination_folder: str, file_name: str, progress_callback, lang_dict: dict):
    ...
    try:
        ...
        download_file_with_retries(url, file_path, session=get_session(), timeout=60, chunk_size=65536, on_progress=_report)
...
def get_mod_metadata_from_jar(jar_path: str, lang_dict: dict):
    metadata = {'name': os.path.basename(jar_path), 'version': 'Unknown', 'game_version': 'Unknown', 'mod_id': None, 'author': 'Unknown', 'modrinth_project_id': None, 'icon_path_in_jar': None, 'icon_data': None}
    try:
        with zipfile.ZipFile(jar_path, 'r') as jar:
            if 'fabric.mod.json' in jar.namelist():
                with jar.open('fabric.mod.json') as f:
...
def scan_local_mods(mods_folder: str, lang_dict: dict, installed_mods_data: dict):
    ...
    if not os.path.exists(mods_folder):
        ...
        for filename in os.listdir(mods_folder):
            ...
            if not filename.endswith(('.jar', '.jar.disabled')):
                continue
            else:
        ...
        if project_ids_to_fetch:
            try:
                ...
                response = request_with_retries('GET', f'{MODRINTH_API_URL}/projects', params=params, headers=_get_headers(), timeout=15)
                response.raise_for_status()
