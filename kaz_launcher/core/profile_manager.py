...
def create_launcher_profiles_if_needed(minecraft_dir: str, client_token: str):
    ...
    if not os.path.exists(profiles_path):
        base_structure = {'profiles': {}, 'settings': {'locale': 'ru_ru', 'enableSnapshots': True, 'enableAdvanced': False}, 'version': 2, 'clientToken': client_token}
...
def add_profile(minecraft_dir: str, version_id: str, profile_name: str, icon: str='Furnace'):
    ...
    if not os.path.exists(profiles_path):
        ...
        with open(profiles_path, 'r+', encoding='utf-8') as f:
            ...
            new_profile = {'created': now_iso, 'icon': icon, 'lastUsed': now_iso, 'lastVersionId': version_id, 'name': profile_name, 'type': 'custom'}
